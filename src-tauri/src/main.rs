...
use ates_autonomous::types::CotEntry;
...
impl AppState {
    ...
    async fn push_tauri_cot(&self, agent: &str, input: &str, action: &str, reason: &str, confidence: f64, symbol: Option<String>) {
        ...
        store.push(entry);
        if store.len() > 500 { store.remove(0); }
    }
}
...
async fn start_autonomous_system(state: State<'_, Mutex<AppState>>) -> Result<serde_json::Value, String> {
    ...
    // NOTE: in-process orchestrator is initialized in main() — no need to create here.
    // The subprocess orchestrator below runs independently for background monitoring.
    ...
    if processes.orchestrator.is_none() {
        ...
        match spawn_result {
            Ok(child) => {
                ...
                println!("[Tauri] ✅ Orchestrator subprocess spawned");
            }
            Err(e) => {
...
async fn execute_trade(
    ...
    if !check.passed {
        app_state.push_tauri_cot(
            "DisciplineCore",
            &format!("Discipline check for {} {} @ {:.2}", symbol, direction_str, entry_price),
            "REJECTED",
            &check.reasons.join("; "),
            0.0,
            Some(symbol.clone()),
    ...
    if entry_price <= 0.0 || stop_loss <= 0.0 || take_profit <= 0.0 {
        app_state.push_tauri_cot(
            ...
            &format!("Price validation for {} {} @ {:.2}", symbol, direction_str, entry_price),
            "REJECTED",
    ...
    if (direction == TradeDirection::Long && stop_loss >= entry_price) ||
       (direction == TradeDirection::Short && stop_loss <= entry_price) {
        app_state.push_tauri_cot(
            "StrategyDecisionAgent",
            &format!("Stop validation for {} {} @ {:.2} SL: {:.2}", symbol, direction_str, entry_price, stop_loss),
            "REJECTED",
    ...
    let result = match app_state.execution.execute_setup(setup, &rules).await {
        Ok(true) => {
            app_state.push_tauri_cot(
                "ExecutionEngine",
                &format!("Execute {} {} @ {:.2} | SL: {:.2} | TP: {:.2}", symbol, direction_str, entry_price, stop_loss, take_profit),
                "FILLED",
                &format!("Trade executed: {} {} @ {:.2}", symbol, direction_str, entry_price),
                0.85,
                Some(symbol.clone()),
            ).await;
            Ok(format!("TRADE EXECUTED SUCCESSFULLY: {} @ {}", symbol, entry_price))
        }
        Ok(false) => {
            app_state.push_tauri_cot(
                "ExecutionEngine",
...
async fn update_rules(
    ...
    app_state.push_tauri_cot(
        ...
        "UPDATED",
        &format!("Confluence: {}, SessionTiming: {}", use_confluence, respect_session_timing),
        0.9,
        None,
    ).await;
