...
use tredo_core::{TradeDirection, OhlcvBar, PivotMethod, calculate_pivot_points, calculate_confluence_score, MarketContext, Agent};
use tredo_core::episode::{TradingEpisode, MarketStateSnapshot, ReasoningStep};
...
pub async fn fetch_binance_price(client: &reqwest::Client, symbol: &str) -> Result<f64, Box<dyn std::error::Error + Send + Sync>> {
...
pub async fn fetch_kraken_price(client: &reqwest::Client, symbol: &str) -> Result<f64, Box<dyn std::error::Error + Send + Sync>> {
...
pub async fn fetch_coinbase_price(client: &reqwest::Client, symbol: &str) -> Result<f64, Box<dyn std::error::Error + Send + Sync>> {
    let product = format!("{}-USDT", symbol);
    let url = format!("https://api.coinbase.com/api/v3/brokerage/market/products/{}", product);
    let resp: serde_json::Value = client.get(&url)
        .header("User-Agent", "Mozilla/5.0")
        .timeout(Duration::from_secs(5))
        .send().await?.json().await?;
...
pub async fn fetch_coingecko_price(client: &reqwest::Client, symbol: &str) -> Result<f64, Box<dyn std::error::Error + Send + Sync>> {
...
pub async fn fetch_binance_24h_ticker(client: &reqwest::Client, symbol: &str) -> Result<serde_json::Value, Box<dyn std::error::Error + Send + Sync>> {
    ...
    if !p.open_positions.is_empty() {
        for pos in &p.open_positions {
            println!("   • {} {} {} qty={:.0} entry={:.2} cur={:.2} P&L=₹{:.2}",
                pos.symbol, if pos.direction == tredo_core::TradeDirection::Long { "LONG" } else { "SHORT" },
