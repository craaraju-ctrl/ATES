...
impl LoopManager {
    fn new(orchestrator: tredo_autonomous::AutonomousOrchestrator, client: reqwest::Client, assets: Vec<String>) -> Self {
        Self {
            orchestrator,
            client,
            assets,
...
async fn initialize_system(
    ...
    println!("[Orchestrator] 🌐 Initializing all data feeds for watchlist: {:?}", assets);
...

    {
        ...
        *summary = format!("System initialized. Monitoring: {} with Ollama + Kronos.", monitored);
    }
...
async fn get_available_models(
    ...
    let base_url = endpoint.replace("/api/generate", "").replace("/api/chat", "");
    ...
    match res {
        Ok(resp) if resp.status().is_success() => {
            ...
            if let Ok(tags_res) = resp.json::<OllamaTagsResponse>().await {
                let models: Vec<serde_json::Value> = tags_res.models.into_iter().map(|m| {
    ...
    match state.memory.load_state("watchlist/state") {
        Ok(Some(json)) => {
            if let Ok(restored) = serde_json::from_str::<Vec<String>>(&json) {
                ...
                *watchlist = restored;
                println!("[Restore] ✅ Watchlist restored from redb: {:?}", *watchlist);
            }
        }
        _ => {
...
async fn add_to_watchlist(
    ...
    if symbol.is_empty() {
        return (axum::http::StatusCode::BAD_REQUEST, Json(serde_json::json!({ "error": "Symbol cannot be empty" })));
    }
    
    let scanner = tredo_autonomous::scanner::WatchlistScannerAgent::new(state.orchestrator.state.clone());
    ...
    (axum::http::StatusCode::OK, Json(serde_json::json!({ "added": added, "symbol": symbol })))
}
...
async fn remove_from_watchlist(
    ...
    (axum::http::StatusCode::OK, Json(serde_json::json!({ "removed": removed, "symbol": symbol })))
}
...
async fn execute_trade(
    ...
    use tredo_core::{TradeDirection, TradeSetup, validate_trade_setup};
...
async fn get_crypto_prices(
    ...
    let symbols: Vec<&str> = req.symbols.as_deref().unwrap_or("BTC,ETH,SOL,BNB,XRP,ADA,DOGE,AVAX")
        .split(',')
        .map(str::trim)
        .filter(|s| !s.is_empty())
        .collect();
    ...
    for sym in symbols {
        ...
        match price_result {
            Ok(p) => { results.insert(sym.to_string(), serde_json::json!({ "price": p, "exchange": exchange })); }
            Err(e) => { results.insert(sym.to_string(), serde_json::json!({ "error": e.to_string() })); }
        }
    }
...
async fn get_crypto_market(
    ...
    let client = reqwest::Client::new();
    let sym = q.get("symbol").map(|s| s.to_uppercase()).unwrap_or_else(|| "BTC".to_string());

    // Fetch 24h stats from Binance
    let binance_data = loops::fetch_binance_24h_ticker(&client, &sym).await.unwrap_or_default();
    let price = binance_data["lastPrice"].as_str().and_then(|s| s.parse::<f64>().ok()).unwrap_or(0.0);
    let change_pct = binance_data["priceChangePercent"].as_str().and_then(|s| s.parse::<f64>().ok()).unwrap_or(0.0);
    let high = binance_data["highPrice"].as_str().and_then(|s| s.parse::<f64>().ok()).unwrap_or(0.0);
    let low = binance_data["lowPrice"].as_str().and_then(|s| s.parse::<f64>().ok()).unwrap_or(0.0);
    let volume = binance_data["volume"].as_str().and_then(|s| s.parse::<f64>().ok()).unwrap_or(0.0);
    let quote_vol = binance_data["quoteVolume"].as_str().and_then(|s| s.parse::<f64>().ok()).unwrap_or(0.0);

    // Cross-exchange comparison (async parallel)
    let sym_clone = sym.clone();
    let client2 = client.clone();
    let coingecko_price = tokio::spawn(async move {
        loops::fetch_coingecko_price(&client2, &sym_clone).await.unwrap_or(0.0)
    });
...
async fn main() {
    ...
    // ======================================================================
    {
        ...
        if started {
            ...
            println!("[Orchestrator]    • Use Ctrl+C (or the Stop button in UI) to shut down cleanly");
        }
    }
            ...
            match tokio::net::TcpListener::bind(try_addr).await {
                Ok(l) => {
                    if current_port != port {
                        println!("[Orchestrator] ⚠ Port {} in use, using {} instead", port, current_port);
                    }
                    break l;
                }
                Err(e) if current_port < port + 10 => {
                    current_port += 1;
                    eprintln!("[Orchestrator] Port {} bind failed ({}), trying next...", current_port-1, e);
                    continue;
                }
                Err(e) => {
