...
impl Agent for OvertradingPreventerAgent {
    ...
    async fn run(&self, _input: Option<AgentInput>) -> Result<AgentOutput, Box<dyn Error + Send + Sync>> {
        ...
        println!("[OvertradingPreventer] Trades today: {}/{} | Status: {}",
            total_trades, limit, if passed { "OK" } else { "BLOCKED" });

        let check = DisciplineCheck {
            passed,
            reasons: if passed { vec![] } else { vec![format!("Overtrading limit reached: {} trades", total_trades)] },
