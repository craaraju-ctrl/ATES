...
impl SharedState {
    pub fn new(memory: MemoryStore, rules: DisciplineRules, config: Config) -> Result<Self, rusqlite::Error> {
        // Open (or create) SQLite history database (with built-in recovery for locks/WAL)
        let episode_store = Arc::new(EpisodeStore::open("tredo_history.db") ? );
        ...
        Ok(Self {
            ...
            vector_memory: Arc::new(tokio::sync::Mutex::new(VectorMemory::new("tredo_vectors.json"))),
