...
impl PortfolioManagerAgent {
        ...
        for pos in &mut portfolio.open_positions {
            if pos.symbol == symbol {
                ...
                pos.unrealized_pnl = match pos.direction {
                    tredo_core::TradeDirection::Long => (current_price - pos.entry_price) * pos.quantity,
                    tredo_core::TradeDirection::Short => (pos.entry_price - current_price) * pos.quantity,
                };
        ...
        if updated {
            let open_value: f64 = portfolio.open_positions.iter()
                .map(|p| match p.direction {
                    ...
                    tredo_core::TradeDirection::Short => (p.quantity * p.entry_price) + p.unrealized_pnl,
                })
                .sum();
    ...
    pub async fn add_position(&self, signal: &TradeSignal) -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        println!(
            ...
            signal.symbol,
            if signal.direction == tredo_core::TradeDirection::Long { "LONG" } else { "SHORT" },
...
mod tests {
    ...
    async fn test_short_position_accounting() {
        ...
        let memory = MemoryStore::new(db_path).expect("Portfolio operation failed - check for concurrent access or DB issues");
        ...
        pm.add_position(&signal).await.expect("Portfolio operation failed - check for concurrent access or DB issues");
        ...
        pm.update_position_pnl("NIFTY", 80.0).await.expect("Portfolio operation failed - check for concurrent access or DB issues");
        ...
        let pnl = pm.close_position("NIFTY", 80.0).await.expect("Portfolio operation failed - check for concurrent access or DB issues");
        assert_eq!(pnl, 200.0);
