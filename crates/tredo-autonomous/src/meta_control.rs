...
use tredo_core::{Agent, AgentTier, AgentInput, AgentOutput, DisciplineRules};
...
impl MetaControlAgent {
    ...
    pub async fn weekly_review(&self, days_back: i64) -> Result<WeeklyReviewReport, Box<dyn Error + Send + Sync>> {
        println!("\n[MetaControl] 📊 Starting weekly review (last {} days)...", days_back);

        let since = Utc::now() - Duration::days(days_back);

        // Load high-regret events from SQLite (primary source)
        let regret_events = self.state.episode_store
        ...
        if total_episodes == 0 {
            ...
            let stored = self.state.memory.load_episodes_since(since_ts).unwrap_or_default();
            if stored.is_empty() {
        ...
        let high_regret_summaries: Vec<String> = regret_events.iter().map(|ev| {
            format!(
        ...
        if high_regret_summaries.is_empty() {
            ...
            return Ok(WeeklyReviewReport {
                ...
                proposed_changes: vec![],
                changes_applied: false,
                summary: format!("Reviewed {} regret events. No issues found.", total_episodes),
            });
        }

        println!("[MetaControl] 🔍 Found {} high-regret episodes out of {}. Asking LLM for analysis...",
        ...
        let analysis = self.state.llm.ask_for_meta_review(
            &high_regret_summaries,
            &current_rules_summary,
        ).await;

        let pattern = analysis["pattern"].as_str().unwrap_or("No pattern identified").to_string();
        let recommendation = analysis["recommendation"].as_str().unwrap_or("No recommendation").to_string();
        ...
        if let Some(changes) = analysis["suggested_changes"].as_array() {
            for change in changes {
                if let (Some(rule), Some(current), Some(suggested)) = (
                    ...
                    let reason = change["reason"].as_str().unwrap_or("No reason given").to_string();
        ...
        if !proposed_changes.is_empty() {
            ...
            for change in &mut proposed_changes {
                ...
                if applied {
                    ...
                    println!("[MetaControl] ✅ Applied rule change: {} → {:.4} (was {:.4}) — {}",
        ...
        println!("[MetaControl] ✅ Weekly review complete — {} changes applied.", changes_applied as u8);
        Ok(report)
    }
}
...
impl Agent for MetaControlAgent {
    fn name(&self) -> &str { "MetaControlAgent" }
