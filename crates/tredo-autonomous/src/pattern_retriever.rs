...
use std::error::Error;
use chrono::Utc;
use tredo_core::{Agent, AgentTier, AgentInput, AgentOutput};
...
impl PatternRetrieverAgent {
    ...
    pub async fn find_patterns(&self, symbol: &str) -> Result<Vec<PatternMatch>, Box<dyn Error + Send + Sync>> {
        ...
        let similar = self.state.episode_store
        ...
        if !similar.is_empty() {
            ...
            let lessons: Vec<String> = similar.iter()
            ...
            println!("[PatternRetriever] SQLite: {} similar episodes | WR: {:.1}% | avg P&L: ₹{:.0}",
                total, win_rate * 100.0, avg_pnl);
        } else {
            println!("[PatternRetriever] No historical SQLite episodes for {} in {} regime", symbol, regime);
        }

        // ── Fallback: in-memory signal patterns (when history is sparse) ────
        let signals = self.state.last_signals.read().await;
        let relevant: Vec<&TradeSignal> = signals.iter()
            .filter(|s| s.symbol == symbol)
            .collect();

        if !relevant.is_empty() {
            // Pattern: direction bias
            let longs  = relevant.iter().filter(|s| s.direction == tredo_core::TradeDirection::Long).count();
            let shorts = relevant.iter().filter(|s| s.direction == tredo_core::TradeDirection::Short).count();
            ...
            if total > 0 {
                ...
                patterns.push(PatternMatch {
                    ...
                    historical_outcome: format!("LONG {:.0}% / SHORT {:.0}%", long_pct * 100.0, (1.0 - long_pct) * 100.0),
                    avg_return: 0.0,
        ...
        patterns.sort_by(|a, b| b.match_score.partial_cmp(&a.match_score).unwrap_or(std::cmp::Ordering::Equal));

        println!("[PatternRetriever] Found {} patterns for {}", patterns.len(), symbol);
        for p in &patterns {
            println!("[PatternRetriever]   {} | Score: {:.2} | WR: {:.1}% | {}",
                p.pattern_key, p.match_score, p.win_rate * 100.0, p.historical_outcome);
        }
    ...
    pub async fn record_pattern(&self, symbol: &str, key: &str, data: &str) -> Result<(), Box<dyn Error + Send + Sync>> {
        let memory_key = format!("patterns/{}/{}", symbol, key);
        let timestamped = format!("[{}] {}", Utc::now().to_rfc3339(), data);
        self.state.memory.store_decision(&memory_key, &timestamped)?;
        ...
        match input {
            Some(AgentInput::ConfluenceRequest { context }) => {
                ...
                if !patterns.is_empty() {
                    let summary = patterns.iter()
                        .map(|p| format!("{}: score={:.2}", p.pattern_key, p.match_score))
                        .collect::<Vec<_>>()
                        .join(", ");
                    println!("[PatternRetriever] ✅ Pattern analysis complete: {}", summary);
                }
                Ok(AgentOutput::Done)
            }
            _ => {
