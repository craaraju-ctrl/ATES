...
impl OutcomeProcessor {
    ...
    pub async fn close_episode(
        ...
        let (regret_score, lesson, rule_violated) =
            self.score_regret(outcome, confluence_score, consecutive_losses, portfolio_heat, pnl_pct).await;

        let session = {
            use crate::helpers::get_indian_session_info;
            let info = get_indian_session_info(Utc::now());
            if info.market_open { info.session_name } else { "OffHours".to_string() }
        };
        ...
        if regret_score >= 0.5 {
            ...
            if let Err(e) = store.insert_regret_event(&ev) {
                ...
                println!("[OutcomeProcessor] ⚠ HIGH REGRET event logged (score: {:.2}) — {}", regret_score, lesson);
            }
        }
        ...
        if bad_today >= 3 {
            println!("[OutcomeProcessor] 🚨 {} high-regret trades today — triggering MetaControl review", bad_today);
    ...
    async fn score_regret(
        ...
        if outcome == "LOSS" {
            ...
            if consecutive_losses >= 2 {
                ...
                if rule_violated.is_empty() { rule_violated = "max_consecutive_losses".to_string(); }
            }
            ...
            if portfolio_heat > 0.30 {
                ...
                if rule_violated.is_empty() { rule_violated = "portfolio_heat_limit".to_string(); }
            }
            ...
            if pnl_pct < -0.025 {
                ...
                if rule_violated.is_empty() { rule_violated = "max_risk_per_trade".to_string(); }
            }
        } else if outcome == "WIN" {
