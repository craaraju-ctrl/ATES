...
impl Agent for DrawdownMonitorAgent {
    ...
    fn tier(&self) -> tredo_core::AgentTier { tredo_core::AgentTier::Sub }
    ...
    async fn run(&self, _input: Option<AgentInput>) -> Result<AgentOutput, Box<dyn Error + Send + Sync>> {
        ...
        if halted {
            println!("[DrawdownMonitor] 👋 TRADING HALTED | Drawdown: {:.2}% | Limit: {:.2}%", 
                pct, limit_pct);
        } else if pct >= limit_pct * 0.8 {
            println!("[DrawdownMonitor] ⚠️ WARNING | Drawdown: {:.2}% | Remaining: {:.2}% | Limit: {:.2}%", 
                pct, remaining, limit_pct);
        } else {
            println!("[DrawdownMonitor] ✅ OK | Drawdown: {:.2}% | Remaining: {:.2}% | Limit: {:.2}%", 
                pct, remaining, limit_pct);
        }
        ...
        let check = DisciplineCheck {
            passed: !halted,
            reasons: if halted { 
                vec![format!("Drawdown limit reached: {:.2}% / {:.2}%", pct, limit_pct)] 
            } else { vec![] },
