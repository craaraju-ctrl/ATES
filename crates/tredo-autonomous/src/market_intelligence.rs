...
impl MarketIntelligenceAgent {
    ...
    pub async fn analyze_market(&self, symbol: &str, price: f64) -> Result<(f64, tredo_core::PivotLevels), Box<dyn Error + Send + Sync>> {
        // === UPGRADE: Integrate new skills/tools for richer MI (sentiment, vol, regime, corr)
        let sentiment = crate::sentiment_analyzer::SentimentAnalyzer::new(self.state.clone()).analyze_sentiment(symbol).await;
        let (_vol, expansion) = crate::volatility_calculator::VolatilityCalculator::new(self.state.clone()).compute_volatility(symbol, price).await;
        let regime = crate::regime_detector::RegimeDetector::new(self.state.clone()).detect_regime(symbol, price).await;
        let corr = crate::correlation_checker::CorrelationChecker::new(self.state.clone()).check_correlation(symbol).await;

        // Boost confluence with new signals (research: multi-factor like TradingAgents)
        let extra_score = (sentiment - 0.5) * 0.15 + (if expansion { 0.1 } else { 0.0 }) + (corr - 0.5) * 0.1;
        println!("[MI UPGRADE] {} sentiment={:.2} vol_exp={:?} regime={:?} corr={:.2} extra={:.2}", symbol, sentiment, expansion, regime, corr, extra_score);

        // Wire new skills via AgentSkill trait (upgrade for pluggable tools)
        // In future, collect Vec<Box<dyn tredo_core::skills::AgentSkill>> and execute in parallel.
