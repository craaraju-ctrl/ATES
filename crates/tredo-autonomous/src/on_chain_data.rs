...
impl OnChainData {
    ...
    pub async fn fetch_onchain(&self, symbol: &str) -> f64 {
        ...
        if let Some(bars) = history.get(symbol) {
            ...
            eprintln!("[OnChain] {}: vol={:.2}, price={:.2}, vol_sig={:.2} -> score={:.2}", 
    ...
    fn calculate_volume_signal(&self, bars: &[tredo_core::OhlcvBar]) -> f64 {
        ...
        let price_change = if last_5_prices.len() >= 2 {
            (last_5_prices[0] - last_5_prices[last_5_prices.len()-1]) / last_5_prices[last_5_prices.len()-1]
        } else { 0.0 };
        ...
        let vol_score = if volume_ratio > 1.2 {
            ...
            if price_change > 0.01 { 0.7 }  // High volume + up = strong accumulation
            else if price_change < -0.01 { 0.3 }  // High volume + down = distribution
            else { 0.5 }
        } else if volume_ratio < 0.8 {
            // Low volume - potential accumulation (smart money)
            if price_change > 0.0 { 0.6 } else { 0.4 }
        } else {
            // Normal volume
    ...
    fn calculate_price_signal(&self, bars: &[tredo_core::OhlcvBar]) -> f64 {
        ...
        let change_5 = (prices[0] - prices[4.min(prices.len()-1)]) / prices[4.min(prices.len()-1)];
        let change_10 = (prices[0] - prices[9.min(prices.len()-1)]) / prices[9.min(prices.len()-1)];
        
        // Trend alignment - if all timeframes agree, strong signal
        let mut score: f64 = 0.5;
        
        if change_5 > 0.02 { score += 0.15; }
        else if change_5 < -0.02 { score -= 0.15; }
        
        if change_10 > 0.05 { score += 0.2; }
        else if change_10 < -0.05 { score -= 0.2; }
        ...
        if prices[0] > ma5 { score += 0.1; }
        if prices[0] > ma10 { score += 0.1; }
    ...
    fn calculate_volatility_signal(&self, bars: &[tredo_core::OhlcvBar]) -> f64 {
        ...
        let older: Vec<f64> = bars.iter().rev().skip(10).take(10).map(|b| b.high - b.low).collect();
