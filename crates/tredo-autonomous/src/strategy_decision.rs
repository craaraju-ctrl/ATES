...
use crate::debate::run_debate; // Full debate aggregator wired (Proposer etc using new skills)
...
impl StrategyDecisionAgent {
    ...
    pub async fn generate_signal(
        ...
        let mtf_patterns_context = {
            ...
            match mtf.get(symbol) {
                Some(p) if !p.timeframes_with_patterns.is_empty() => tredo_core::format_mtf_confirmation(p),
                _ => String::new(),
            }
        };
        ...
        let (debate_action, debate_conf, debate_reason) = run_debate(self.state.clone(), &debate_input).await;

        // Fallback or augment with LLM only if debate uncertain (keeps selective LLM usage)
        let decision = if debate_conf > 0.7 {
            // Use debate result
            tredo_core::LlmTradeDecision { action: debate_action.clone(), entry, sl: entry*0.99, tp: entry*1.02, reason: debate_reason.clone() }
        } else {
            self.state.llm.ask_for_trade_decision(
        ...
        // shared with coding agents (Grok/Hermes via same store). Use in debate/LLM prompts.
        {
            ...
            if !past.is_empty() {
                println!("[StrategyDecision] Recalled from agentmemory for {}: {} entries", symbol, past.len());
            }
            let mem_content = format!("{}: {} | {} | Mode: {} | Reason: {}", symbol, decision.action, decision.reason, trading_mode, decision.reason);
...
// TODO: integrate AgentMemoryClient::new().remember( &format!("LLM decision for {}: {}", symbol, decision), "decision").await; to persist beyond current episode store.

// agentmemory integration for persistent runtime memory in TREDO agents
// This gives the autonomous trading agents (beyond current sqlite episodes/COT)
// infinite long-term memory via the shared agentmemory store.
// After debate/decision, remember key outcomes.
// Before LLM call, recall similar past decisions for better context (mitigates small model limitations).
use crate::agentmemory::AgentMemoryClient;  // assumes mod agentmemory; pub use in lib or direct

// Example: in the decision function, after computing decision:
async fn remember_decision(symbol: &str, action: &str, reason: &str) {
    if let Ok(mem) = std::future::ready(AgentMemoryClient::new()).await {  // adjust for async context
        let content = format!("{} {}: {} (nemotron debate, conservative mode)", symbol, action, reason);
        let _ = mem.remember(&content, "trading_decision").await;
    }
}
