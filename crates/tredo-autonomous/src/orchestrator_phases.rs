...
impl crate::orchestrator_struct::AutonomousOrchestrator {
    pub async fn phase1_discipline_checks(&self) -> Result<bool, Box<dyn Error + Send + Sync>> {
        ...
        println!("[PHASE 1] Session: {} | Drawdown: {} | RedFolder: {} | Overtrading: {}",
    ...
    pub async fn phase2_market_analysis(&self, symbol: &str, price: f64) -> Result<(f64, PivotLevels), Box<dyn Error + Send + Sync>> {
        ...
        let _ = self.pivot_calc.run(Some(tredo_core::AgentInput::PivotRequest { high: price*1.01, low: price*0.99, close: price })).await;
    ...
    pub async fn phase3_risk_assessment(&self, symbol: &str, price: f64) -> Result<crate::types::RiskAnalysis, Box<dyn Error + Send + Sync>> {
        ...
        let analysis = self.risk_psych.analyze_risk(&tredo_core::MarketContext {
