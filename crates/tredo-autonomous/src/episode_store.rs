...
impl EpisodeStore {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self, rusqlite::Error> {
        ...
        for attempt in 0..4 {
            ...
            if attempt > 0 {
                ...
                eprintln!("[EpisodeStore] Recovery attempt {} for {} (cleaned WAL/SHM)", attempt + 1, path.display());
            ...
            match Connection::open(path) {
                Ok(conn) => {
                    ...
                    if attempt > 0 {
                        eprintln!("[EpisodeStore] ✅ SQLite recovered after {} attempts (WAL mode)", attempt + 1);
                    } else {
                ...
                Err(e) => {
                    ...
                    last_err = Some(e);
                    if msg.contains("lock") || msg.contains("busy") || msg.contains("database is locked") {
                        continue;
                    }
                    if attempt < 2 {
                        continue;
        ...
        Err(last_err.unwrap_or_else(|| rusqlite::Error::SqliteFailure(rusqlite::ffi::Error::new(5 /* BUSY */), Some("unrecoverable lock".into()))))
    }
    ...
    pub fn insert_closed_trade(&self, ep: &ClosedEpisode) -> Result<(), rusqlite::Error> {
        let conn = self.conn.lock().expect("SQLite connection lock poisoned - this indicates a previous panic in DB code");
        conn.execute(
    ...
    pub fn flush_cot_batch(&self, rows: &[CotLogRow]) -> Result<(), rusqlite::Error> {
        ...
        for r in rows {
            tx.execute(
                ...
                params![r.chain_id, r.agent, r.action, r.reason, r.confidence, r.symbol, r.ts],
            )?;
        }
        tx.commit()?;
        Ok(())
    ...
    pub fn insert_rule_change(&self, rc: &RuleChangeRow) -> Result<(), rusqlite::Error> {
        ...
        conn.execute(
            "INSERT INTO rule_changes (rule_name, old_value, new_value, reason, applied_at)
             VALUES (?1, ?2, ?3, ?4, ?5)",
            params![rc.rule_name, rc.old_value, rc.new_value, rc.reason, rc.applied_at],
        )?;
        Ok(())
    }
