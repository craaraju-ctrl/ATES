...
use std::error::Error;
use chrono::Utc;
use tredo_core::{Agent, AgentTier, AgentInput, AgentOutput, TradingEpisode, TradeOutcome};
...
impl ExecutionCoordinatorAgent {
    ...
    pub async fn execute_paper_trade(&self, signal: &TradeSignal) -> Result<String, Box<dyn Error + Send + Sync>> {
        ...
        let direction_str = if signal.direction == tredo_core::TradeDirection::Long { "BUY" } else { "SELL" };
        let exec_log = format!(
