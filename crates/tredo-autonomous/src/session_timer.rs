...
use std::error::Error;
use chrono::Utc;
use tredo_core::{Agent, AgentTier, AgentInput, AgentOutput, DisciplineCheck};
...
impl SessionTimerAgent {
    ...
    fn is_nse_session_active(&self) -> (bool, String) {
        ...
        if !session.market_open {
            return (false, format!("Market CLOSED | Session: {} | Next open: {:?}",
        ...
        if session.minutes_since_open < 15 {
            return (false, format!("WAIT | Only {} mins since open - avoid opening volatility",
        ...
        if let Some(mins_to_close) = session.time_to_close {
            ...
            if mins < 15 {
                return (false, format!("WAIT | Only {} mins to close - avoid closing unwinds", mins));
            }
            return (true, format!("ACTIVE | {} | Mins since open: {} | Mins to close: {}",
