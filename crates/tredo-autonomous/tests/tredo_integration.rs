...
use chrono::Utc;
use std::sync::Arc;
use tredo_core::{Config, DisciplineRules, MemoryStore, TradeDirection, OhlcvBar};
...
fn setup_test_env(db_name: &str) -> (AutonomousOrchestrator, String) {
    ...
    for f in &["tredo_history.db", "tredo_history.db-wal", "tredo_history.db-shm"] {
    ...
    let state = SharedState::new(memory, rules, config).expect("SharedState init (episode DB)");
