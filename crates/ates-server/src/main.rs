//! # ATES HTTP Server — Production Trading Backend
//!
...
//! ## Run
//! ```bash
//! cargo run -p ates-server -- --port 8080
//! ```

use ates_core::paper_engine::*;
use axum::{
    extract::{
...
async fn handle_close_position(
    ...
    let broker = state.registry.active_broker().await;
    match broker.close_position(&req.position_id, req.exit_price).await {
        Ok(t) => Json(ApiResponse::ok(t)),
        Err(e) => ApiResponse::err(&e),
    }
}
...
async fn handle_broker_config(
    ...
    match req.broker.to_lowercase().as_str() {
        "zerodha" | "kite" => {
            let base_url = req.base_url.unwrap_or_else(|| "https://api.kite.trade".to_string());
            let broker = Arc::new(ZerodhaKiteBroker::new(&req.api_key, &req.api_secret, &base_url));
            state.registry.register_live_broker(broker).await;
            Json(ApiResponse::ok("Zerodha Kite broker registered".to_string()))
        }
        "angel" => {
...
async fn handle_broker_test(
    State(state): State<Arc<AppState>>,
) -> Json<ApiResponse<String>> {
    let broker = state.registry.active_broker().await;
    match broker.connect().await {
        Ok(()) => Json(ApiResponse::ok(format!("Connected to {}", broker.broker_name()))),
        Err(e) => ApiResponse::err(&e),
    }
}
...
async fn handle_ws_socket(socket: WebSocket, state: Arc<AppState>) {
    ...
    let initial_state = serde_json::json!({
        ...
        "summary": broker.get_summary().await.unwrap_or_default(),
        "positions": broker.get_positions().await.unwrap_or_default(),
    });
    let _ = sender.send(Message::Text(serde_json::to_string(&initial_state).unwrap())).await;
...
async fn main() {
    ...
    log::info!("[ATES Server] Starting production trading backend...");
    ...
    registry.set_mode(TradingMode::Paper).await.expect("Failed to initialize paper broker");
    ...
    let app = Router::new()
        ...
        .route_service("/", get(|| async { axum::response::Redirect::to("/index.html") }))
    ...
    log::info!("[ATES Server] Listening on http://{}", addr);
