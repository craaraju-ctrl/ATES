...
impl AutonomousOrchestrator {
    pub fn new(state: SharedState) -> Self {
        Self {
            ...
            risk_psych: Arc::new(crate::risk_psychology::RiskPsychologyAgent::new(state.clone())),
            reflector: Arc::new(crate::reflector::ReflectorAgent::new(state.clone())),
            strategy: Arc::new(crate::strategy_decision::StrategyDecisionAgent::new(state.clone())),
            portfolio: Arc::new(crate::portfolio_manager::PortfolioManagerAgent::new(state.clone())),
            execution: Arc::new(crate::execution_coordinator::ExecutionCoordinatorAgent::new(state.clone())),
            risk_calc: Arc::new(crate::risk_calculator::RiskCalculatorAgent::new(state.clone())),
            pivot_calc: Arc::new(crate::pivot_calculator::PivotCalculatorAgent::new(state.clone())),
            confluence: Arc::new(crate::confluence_scorer::ConfluenceScorerAgent::new(state.clone())),
            session_timer: Arc::new(crate::session_timer::SessionTimerAgent::new(state.clone())),
            drawdown: Arc::new(crate::drawdown_monitor::DrawdownMonitorAgent::new(state.clone())),
            red_folder: Arc::new(crate::red_folder_checker::RedFolderCheckerAgent::new(state.clone())),
    ...
    pub async fn record_result(&self, phase: &str, passed: bool, details: Vec<String>, duration_ms: u64) {
