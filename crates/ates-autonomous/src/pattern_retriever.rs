...
impl PatternRetrieverAgent {
    ...
    pub async fn find_patterns(&self, symbol: &str) -> Result<Vec<PatternMatch>, Box<dyn Error + Send + Sync>> {
        ...
        if !relevant.is_empty() {
            ...
            let longs  = relevant.iter().filter(|s| s.direction == ates_core::TradeDirection::Long).count();
            let shorts = relevant.iter().filter(|s| s.direction == ates_core::TradeDirection::Short).count();
