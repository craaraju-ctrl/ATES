...
use std::sync::Arc;
use ates_core::{Agent, TradeDirection};
use crate::types::{TradeSignal, RiskAnalysis};
use std::error::Error;
...
impl Tredo {
    ...
    pub async fn run_identifier(
        ...
        symbol: &str,
        price: f64,
    ) -> Result<(bool, f64, ates_core::PivotLevels), Box<dyn Error + Send + Sync>> {
        println!("[Tredo::Identifier] Scanning market for {} @ {:.2}", symbol, price);

        // Run scanner
        let _ = self.identifier.scanner.scan_watchlist().await?;

        // Run market intelligence
        let (confluence, pivots) = self.identifier.market_intel.analyze_market(symbol, price).await?;

        // Run pivot calculator
        let _ = self.identifier.pivot_calc.run(
            Some(ates_core::AgentInput::PivotRequest {
                high: price * 1.01,
                low: price * 0.99,
                close: price,
            })
        ...
        if !discipline_ok {
            println!("[Tredo::Identifier] ⚠ Discipline checks failed (session: {}, red_folder: {})", session_ok, red_ok);
        }

        println!("[Tredo::Identifier] ✅ Analysis complete — Confluence: {:.1}%, Discipline: {}",
            confluence * 100.0, if discipline_ok { "OK" } else { "FAIL" });
    ...
    pub async fn run_verifier(
        ...
        println!("[Tredo::Verifier] Validating risk for {} @ {:.2} (equity: {:.2})", symbol, price, equity);
        ...
        let analysis = self.verifier.risk_psych.analyze_risk(&ates_core::MarketContext {
        ...
        println!("[Tredo::Verifier] ✅ Risk check — Recommendation: {:?}, Heat: {:.1}%",
            analysis.recommendation, analysis.portfolio_heat * 100.0);
        Ok(analysis)
    }
    ...
    pub async fn run_executer(
        ...
        println!("[Tredo::Executer] Making decision for {} (direction: {:?})", symbol, direction);

        // Strategy decision (LLM)
        let signal_opt = self.executer.strategy.generate_signal(symbol, direction, entry, stop, target).await?;

        match &signal_opt {
            Some(sig) => {
                println!("[Tredo::Executer] ✅ LLM decided {:?} {} @ {:.2} (conf: {:.1}%)",
                    sig.direction, symbol, sig.entry_price, sig.confidence_score * 100.0);
