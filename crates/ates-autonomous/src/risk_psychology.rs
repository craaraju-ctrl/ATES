...
use ates_core::{
    ...
    MarketContext, check_risk_limits,
};
...
impl RiskPsychologyAgent {
    ...
    pub async fn analyze_risk(&self, _context: &MarketContext) -> Result<RiskAnalysis, Box<dyn Error + Send + Sync>> {
        ...
        let total_risk: f64 = portfolio.open_positions.iter()
            .map(|p| p.risk_amount)
            .sum();
        ...
        println!(
            ...
            recommendation, portfolio_heat * 100.0, daily_dd * 100.0, portfolio.total_trades_today
        );
