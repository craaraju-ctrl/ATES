...
//
// Run: cargo test -p ates-autonomous --test tredo_integration

use std::fs;
use chrono::Utc;
use std::sync::Arc;
use ates_core::{Config, DisciplineRules, MemoryStore, TradeDirection, OhlcvBar};
use ates_autonomous::{
    SharedState, AutonomousOrchestrator,
    Tredo, Identifier, Verifier, Executer, Guardian,
    types::{TradeSignal, RiskRecommendation},
};
...
///
/// Cleans up the shared SQLite `ates_history.db` (opened by SharedState::new)
/// plus any WAL/SHM sidecar files before creating the new env.
fn setup_test_env(db_name: &str) -> (AutonomousOrchestrator, String) {
    // Clean up shared SQLite database (opened by SharedState::new internally)
    for f in &["ates_history.db", "ates_history.db-wal", "ates_history.db-shm"] {
...
async fn test_tredo_hierarchy_integrity() {
    ...
    assert!(Arc::ptr_eq(&orch.scanner, &tredo.identifier.scanner), "Tredo scanner should share orchestrator scanner Arc");

    // Verify each group's describe() returns the expected tree
    let ident_desc = Identifier::describe();
    assert!(ident_desc.contains("WatchlistScannerAgent"), "Identifier should contain scanner");
    assert!(ident_desc.contains("MarketIntelligenceAgent"), "Identifier should contain market intel");
    assert!(ident_desc.contains("PivotCalculatorAgent"), "Identifier should contain pivot calc");
    assert!(ident_desc.contains("ConfluenceScorerAgent"), "Identifier should contain confluence");
    assert!(ident_desc.contains("PatternRetrieverAgent"), "Identifier should contain pattern retriever");
    assert!(ident_desc.contains("SessionTimerAgent"), "Identifier should contain session timer");
    assert!(ident_desc.contains("RedFolderCheckerAgent"), "Identifier should contain red folder");

    let ver_desc = Verifier::describe();
    assert!(ver_desc.contains("RiskPsychologyAgent"), "Verifier should contain risk psychology");
    assert!(ver_desc.contains("RiskCalculatorAgent"), "Verifier should contain risk calc");
    assert!(ver_desc.contains("ReflectorAgent"), "Verifier should contain reflector");

    let exec_desc = Executer::describe();
    assert!(exec_desc.contains("StrategyDecisionAgent"), "Executer should contain strategy");
    assert!(exec_desc.contains("PortfolioManagerAgent"), "Executer should contain portfolio manager");
    assert!(exec_desc.contains("ExecutionCoordinatorAgent"), "Executer should contain execution");

    let guard_desc = Guardian::describe();
    assert!(guard_desc.contains("DrawdownMonitorAgent"), "Guardian should contain drawdown");
    assert!(guard_desc.contains("OvertradingPreventerAgent"), "Guardian should contain overtrading");
    assert!(guard_desc.contains("OutcomeLoggerAgent"), "Guardian should contain outcome logger");

    // Verify Tredo's tree_json is valid
    let tree = Tredo::tree_json();
    assert_eq!(tree["name"], "Tredo", "Root should be Tredo");
    assert_eq!(tree["children"].as_array().unwrap().len(), 4, "Should have 4 children: Identifier, Verifier, Executer, Guardian");
    ...
    assert_eq!(ident_agents + ver_agents + exec_agents + guard_agents, 16, "Total: 16 sub-agents");

    // Verify Arc sharing: all Arcs point to the same heap-allocated agent instances
    assert!(Arc::ptr_eq(&orch.scanner, &tredo.identifier.scanner), "scanner Arc should be shared");
    assert!(Arc::ptr_eq(&orch.market_intel, &tredo.identifier.market_intel), "market_intel Arc should be shared");
    assert!(Arc::ptr_eq(&orch.risk_psych, &tredo.verifier.risk_psych), "risk_psych Arc should be shared");
    assert!(Arc::ptr_eq(&orch.execution, &tredo.executer.execution), "execution Arc should be shared");
    assert!(Arc::ptr_eq(&orch.drawdown, &tredo.guardian.drawdown), "drawdown Arc should be shared");
    assert!(Arc::ptr_eq(&orch.outcome_logger, &tredo.guardian.outcome_logger), "outcome_logger Arc should be shared");
...
async fn test_identifier_group() {
    ...
    assert!(result.is_ok(), "Identifier should complete without error: {:?}", result.err());
    ...
    assert!(discipline_ok, "Crypto symbols should bypass session checks → discipline OK");

    // Confluence should be a valid f64 between 0.0 and 1.0
    assert!(confluence >= 0.0, "Confluence should be >= 0.0, got {}", confluence);
    assert!(confluence <= 1.0, "Confluence should be <= 1.0, got {}", confluence);
    println!("  Confluence: {:.2}%", confluence * 100.0);

    // Pivots should have valid finite values
    assert!(pivots.pivot.is_finite(), "Pivot should be finite: {:?}", pivots.pivot);
    assert!(pivots.r1.is_finite(), "R1 should be finite: {:?}", pivots.r1);
    assert!(pivots.s1.is_finite(), "S1 should be finite: {:?}", pivots.s1);
    println!("  Pivot: {:.2}, R1: {:.2}, S1: {:.2}", pivots.pivot, pivots.r1, pivots.s1);

    // Ensure state was updated (market regime should be set)
    let regime = orch.state.market_regime.read().await;
    assert!(regime.is_some(), "Market regime should be set after Identifier run");
...
async fn test_identifier_without_data() {
    ...
    assert!(result.is_ok(), "Identifier should handle missing OHLCV data gracefully: {:?}", result.err());
    let (discipline_ok, confluence, pivots) = result.unwrap();
    ...
    assert!(pivots.pivot.is_finite(), "Pivot should be calculated even with fallback data");

    println!("  No-data test — discipline: {}, confluence: {:.2}%, pivot: {:.2}",
        if discipline_ok { "OK" } else { "FAIL" },
        confluence * 100.0, pivots.pivot);
...
async fn test_verifier_clean_portfolio() {
    ...
    assert!(result.is_ok(), "Verifier should complete without error: {:?}", result.err());
    let analysis = result.unwrap();

    // Clean portfolio should recommend Proceed
    assert_eq!(analysis.recommendation, RiskRecommendation::Proceed,
        "Clean portfolio should get Proceed recommendation, got {:?}", analysis.recommendation);
    assert_eq!(analysis.portfolio_heat, 0.0, "No open positions → zero portfolio heat");
    assert_eq!(analysis.daily_drawdown_pct, 0.0, "No daily P&L → zero drawdown");
    assert!(analysis.max_position_size > 0.0, "Max position size should be positive");
    assert!(analysis.psychology_warnings.is_empty(), "No warnings for clean portfolio");

    println!("  Verifier clean: {:?} | Heat: {:.1}% | MaxPos: ₹{:.2}",
        analysis.recommendation, analysis.portfolio_heat * 100.0, analysis.max_position_size);
...
async fn test_verifier_with_open_position() {
    ...
    orch.portfolio.add_position(&signal).await.expect("Should add position");

    // Verify state is consistent
    assert_eq!(count_positions(&orch.state).await, 1, "Should have 1 open position");
    let equity = get_equity(&orch.state).await;
    assert_eq!(equity, 100_000.0, "Equity unchanged after adding position (value = cash reduction)");

    // Run verifier — should still work with open position
    let result = orch.tredo().run_verifier("NIFTY", 24_500.0, equity).await;
    assert!(result.is_ok(), "Verifier should handle portfolios with open positions");
    let analysis = result.unwrap();

    // With one position, heat should be > 0 but recommendations should still be Proceed
    assert!(analysis.portfolio_heat > 0.0, "Open position → heat should be > 0");
...
async fn test_verifier_drawdown_halt() {
    ...
    assert!(result.is_ok(), "Verifier should handle halted state gracefully");
    let analysis = result.unwrap();

    // Should get Halt recommendation
    assert_eq!(analysis.recommendation, RiskRecommendation::Halt,
        "Halted portfolio should get Halt recommendation");
    assert!(analysis.daily_drawdown_pct > 0.0, "Drawdown should be recorded");

    println!("  Halt test: drawdown {:.1}% | warnings: {}",
        analysis.daily_drawdown_pct * 100.0, analysis.psychology_warnings.len());
...
async fn test_executer_handles_no_llm() {
    ...
    let result = orch.tredo().run_executer(
        "BTC",
        TradeDirection::Long,
        65_000.0,
        64_000.0,
    ...
    match &result {
        Ok(opt) => {
            ...
            match opt {
                Some(sig) => {
                    println!("  LLM available! Decision: {:?} @ {:.2} (conf: {:.1}%)",
                        sig.direction, sig.entry_price, sig.confidence_score * 100.0);
                }
                None => {
...
async fn test_state_sharing_across_groups() {
    ...
    assert!(Arc::ptr_eq(&orch.scanner, &tredo.identifier.scanner), "scanner should be shared");
    assert!(Arc::ptr_eq(&orch.risk_psych, &tredo.verifier.risk_psych), "risk_psych should be shared");
    assert!(Arc::ptr_eq(&orch.strategy, &tredo.executer.strategy), "strategy should be shared");
    ...
    assert!(analysis.psychology_warnings.len() >= 1,
        "Consecutive losses should generate psychology warnings, got {:?}", analysis.psychology_warnings);
...
async fn test_pipeline_state_consistency() {
    ...
    let (discipline_ok, confluence, _pivots) = orch.tredo()
        .run_identifier(symbol, 180.0).await
        .expect("Identifier should succeed");

    assert!(discipline_ok, "SOL is crypto → discipline should pass");
    assert!(confluence > 0.0 || confluence == 0.0, "Confluence should be valid");
    ...
    let equity = get_equity(&orch.state).await;
    let risk = orch.tredo().run_verifier(symbol, 180.0, equity).await
        .expect("Verifier should succeed");

    println!("  Post-verifier recommendation: {:?}, heat: {:.1}%",
        risk.recommendation, risk.portfolio_heat * 100.0);

    // Full pipeline call should complete without panicking
    let summary = orch.run_full_pipeline(
        symbol,
        TradeDirection::Long,
        180.0,
        178.0,
    ...
    match &summary {
        Ok(s) => {
            println!("  Pipeline completed: executed={}, reason={}, duration={}ms",
                s.executed,
...
async fn test_concurrent_group_access() {
    ...
    assert!(ident_result.is_ok(), "Concurrent identifier should succeed: {:?}", ident_result.err());
    assert!(ver_result.is_ok(), "Concurrent verifier should succeed: {:?}", ver_result.err());

    let (disc_ok, conf, _) = ident_result.unwrap();
    println!("  Concurrent identifier: {} | conf: {:.1}%", if disc_ok { "OK" } else { "FAIL" }, conf * 100.0);
    println!("  Concurrent verifier: {:?}", ver_result.unwrap().recommendation);
...
async fn test_multiple_pipeline_runs() {
    ...
    for (i, (symbol, price)) in ["BTC", "ETH", "SOL"].iter().zip([65_000.0, 3_500.0, 180.0].iter()).enumerate() {
        seed_ohlcv(&orch.state, symbol, *price).await;

        let (disc_ok, conf, pivots) = orch.tredo()
            .run_identifier(symbol, *price).await
            .expect("Identifier should succeed");

        println!("  [{}] {}: disc={}, conf={:.1}%, pivot={:.2}", i+1, symbol,
            if disc_ok { "OK" } else { "FAIL" }, conf * 100.0, pivots.pivot);
    }
...
async fn test_portfolio_management_via_tredo() {
    ...
    let equity = get_equity(&orch.state).await;
    let analysis = tredo.run_verifier("NIFTY", 24_500.0, equity).await
        .expect("Verifier should succeed");
    assert_eq!(analysis.recommendation, RiskRecommendation::Proceed);
    ...
    let analysis2 = tredo.run_verifier("NIFTY", 24_500.0, new_equity).await
        .expect("Verifier should succeed on second run");
    assert!(analysis2.portfolio_heat > 0.0, "Portfolio heat should reflect open positions");

    // Close the position
    orch.portfolio.close_position("NIFTY", 24_800.0).await.expect("Should close position");

    // Verify final state
    {
        let portfolio = orch.state.portfolio.read().await;
        assert!(portfolio.open_positions.is_empty(), "All positions should be closed");
        // Close at profit: 24,800 - 24,500 = 300 profit per share, * 1 share = 300
        assert!(portfolio.daily_pnl > 0.0, "Should have positive P&L from profitable close");
        println!("  Final P&L: ₹{:.2}, Equity: ₹{:.2}", portfolio.daily_pnl, portfolio.total_equity);
    }
...
async fn test_cleanup_temp_files() {
    let prefixes = ["hierarchy", "identifier", "identifier_empty", "verifier_clean",
                    "verifier_positions", "verifier_halt", "executer_nollm",
                    "state_sharing", "pipeline_state", "concurrent",
                    "multi_run", "portfolio_tredo"];
