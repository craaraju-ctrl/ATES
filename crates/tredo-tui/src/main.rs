        ...
        if crossterm::event::poll(timeout)? {
            if let Event::Key(key) = event::read()? {
                match key.code {
                    ...
                    KeyCode::BackTab => {
                        app.selected_tab = if app.selected_tab == 0 { 7 } else { app.selected_tab - 1 };
                        app.scroll_offset = 0;
                    }
                    KeyCode::Char(c @ '1'..='8') => {
                        ...
                        if app.selected_tab == Tab::Models as usize && !app.models.is_empty() {
                            if let Some(model) = app.models.get(app.selected_model_index) {
                                if let Some(name) = model.get("name").and_then(|n| n.as_str()) {
                                    // Switch model
                                    if let Some(client) = reqwest::blocking::Client::builder()
...
fn poll_backend(app: &mut AppState) {
    ...
    // Status
    match client.get(format!("{}/api/status")).send() {
    ...
    if let Ok(resp) = client.get(format!("{}/api/cot")).send() {
    ...
    // Agents tree
    if let Ok(resp) = client.get(format!("{}/api/agents")).send() {
        if let Ok(json) = resp.json::<serde_json::Value>() {
    ...
    // Watchlist
    if let Ok(resp) = client.get(format!("{}/api/watchlist")).send() {
    ...
    // Models
    if let Ok(resp) = client.get(format!("{}/api/models")).send() {
        if let Ok(json) = resp.json::<serde_json::Value>() {
...
fn ui(f: &mut ratatui::Frame, app: &mut AppState) {
    ...
    let health = if let Some(s) = &app.status {
        let k = if s.get("kronos").and_then(|v| v.as_bool()).unwrap_or(false) { "🔷" } else { "❌" };
        let o = if s.get("orchestrator").and_then(|v| v.as_bool()).unwrap_or(false) { "⚙️" } else { "❌" };
        let l = if s.get("llm").and_then(|v| v.as_bool()).unwrap_or(false) { "🤖" } else { "❌" };
        let m = app.current_model.as_deref().unwrap_or("unknown");
        format!("K:{} O:{} L:{} | Model: {} | Last: {:?}s ago", k, o, l, m, app.last_poll.map(|t| t.elapsed().as_secs()).unwrap_or(0))
    } else {
    ...
    if let Some(err) = &app.error {
        let err_p = Paragraph::new(err.as_str())
            ...
            .wrap(Wrap { trim: true });
        let err_area = Rect { x: 2, y: size.height.saturating_sub(4), width: size.width - 4, height: 2 };
...
fn render_dashboard(f: &mut ratatui::Frame, area: Rect, app: &AppState) {
    ...
    let equity = status.and_then(|s| s.get("total_equity")).and_then(|v| v.as_f64()).unwrap_or(100_000.0);
    let cash = status.and_then(|s| s.get("cash_balance")).and_then(|v| v.as_f64()).unwrap_or(100_000.0);
    let pnl = status.and_then(|s| s.get("daily_pnl")).and_then(|v| v.as_f64()).unwrap_or(0.0);
    let trades = status.and_then(|s| s.get("total_trades_today")).and_then(|v| v.as_u64()).unwrap_or(0);
    let wins = status.and_then(|s| s.get("winning_trades_today")).and_then(|v| v.as_u64()).unwrap_or(0);
    let losses = status.and_then(|s| s.get("losing_trades_today")).and_then(|v| v.as_u64()).unwrap_or(0);
    ...
    let text = vec![
        Line::from(vec![
            Span::styled("📊 PORTFOLIO", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(""),
        Line::from(vec![
            Span::styled("  EQUITY     ", Style::default().fg(Color::Cyan)),
            Span::styled(format!("₹{:>12.2}", equity), Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(vec![
            Span::styled("  CASH       ", Style::default().fg(Color::Cyan)),
            Span::styled(format!("₹{:>12.2}", cash), Style::default().fg(Color::White)),
        ]),
        Line::from(vec![
            Span::styled("  DAILY P&L  ", Style::default().fg(Color::Cyan)),
            Span::styled(format!("{:>+12.2}", pnl), Style::default().fg(pnl_color).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(""),
        Line::from(vec![
        ...
        Line::from(vec![
            ...
            Span::styled(format!("{:>5.1}%", win_rate), Style::default().fg(if win_rate >= 50.0 { Color::Green } else { Color::Red })),
        ]),
        Line::from(""),
        Line::from(Span::styled("tredo is running autonomously in the background.", Style::default().fg(Color::DarkGray))),
        Line::from(Span::styled("Switch tabs with Tab or number keys. Press r to force a refresh.", Style::default().fg(Color::DarkGray))),
    ];

    let p = Paragraph::new(text)
        .block(Block::default().title("📊 Dashboard").borders(Borders::ALL).border_style(Style::default().fg(Color::Cyan)))
        .wrap(Wrap { trim: true });
...
fn render_cot(f: &mut ratatui::Frame, area: Rect, app: &AppState) {
    let items: Vec<ListItem> = app
        ...
        .map(|entry| {
            ...
            ListItem::new(Line::from(vec![
                Span::styled(format!("[{}] ", &ts[..ts.len().min(19)]), Style::default().fg(Color::DarkGray)),
                Span::styled(format!("{}: ", agent), Style::default().fg(color).add_modifier(Modifier::BOLD)),
    ...
    let list = List::new(items)
        .block(Block::default().title("🔀 Live COT — Chain of Thought").borders(Borders::ALL));
    f.render_widget(list, area);
}

fn render_positions(f: &mut ratatui::Frame, area: Rect, app: &AppState) {
    let status = app.status.as_ref();
    let positions = status.and_then(|s| s.get("open_positions")).and_then(|v| v.as_array()).cloned().unwrap_or_default();

    let lines: Vec<Line> = if positions.is_empty() {
        vec![
            Line::from(Span::styled("No open positions (paper).", Style::default().fg(Color::DarkGray))),
        ]
    } else {
        let mut lines = vec![
            Line::from(vec![
                Span::styled("SYMBOL", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                Span::raw("  "),
                Span::styled("DIR", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                Span::raw("  "),
                ...
                Span::raw("  "),
                Span::styled("P&L", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(""),
        ];
        ...
        for p in positions.iter() {
            ...
            lines.push(Line::from(vec![
                Span::styled(format!("{}", sym), Style::default().fg(Color::White)),
                Span::raw("  "),
                Span::styled(format!("{}", dir), Style::default().fg(if dir == "Long" { Color::Green } else { Color::Red })),
                Span::raw("  "),
...
fn render_watchlist(f: &mut ratatui::Frame, area: Rect, app: &AppState) {
    ...
    let list = List::new(items)
        .block(Block::default().title("👁️ Watchlist (live via /api/watchlist)").borders(Borders::ALL));
    f.render_widget(list, area);
}
...
fn render_models(f: &mut ratatui::Frame, area: Rect, app: &AppState) {
    ...
    let header_text = vec![
        ...
        Line::from(""),
        Line::from(Span::styled("Use ↑/↓ to select, Enter to switch model", Style::default().fg(Color::DarkGray))),
    ];
    
    let header = Paragraph::new(header_text)
        .block(Block::default().borders(Borders::BOTTOM))
        .wrap(Wrap { trim: true });
    f.render_widget(header, Rect { x: area.x, y: area.y, width: area.width, height: 6 });
    
    let list_area = Rect { x: area.x, y: area.y + 6, width: area.width, height: area.height.saturating_sub(7) };
    ...
    let items: Vec<ListItem> = app
        ...
        .map(|(_, m)| {
            ...
            let is_selected = app.models.iter().nth(app.selected_model_index)
...
fn render_tree(f: &mut ratatui::Frame, area: Rect, app: &AppState) {
    ...
    let p = Paragraph::new(text)
        .block(Block::default().title("🌳 Tredo Agent Hierarchy").borders(Borders::ALL))
        .wrap(Wrap { trim: true });
    f.render_widget(p, area);
}
...
fn render_rules(f: &mut ratatui::Frame, area: Rect, app: &AppState) {
    let status = app.status.as_ref();
    let rules = status.and_then(|s| s.get("rules")).cloned().unwrap_or(serde_json::json!({}));

    let p = Paragraph::new(format!("Current rules (from backend):\n\n{:#}", rules))
        .block(Block::default().title("⚖️ Discipline Rules (use `tredo rules k=v` to change)").borders(Borders::ALL))
        .wrap(Wrap { trim: true });
    f.render_widget(p, area);
}
...
"#;
    ...
    let p = Paragraph::new(help)
        .block(Block::default().title("❓ Help & Philosophy").borders(Borders::ALL))
        .wrap(Wrap { trim: true });
    f.render_widget(p, area);
}