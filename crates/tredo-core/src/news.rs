...
impl NewsContext {
    ...
    pub fn to_prompt_string(&self) -> String {
        ...
        let headlines_str: String = self.headlines.iter()
            .take(5)
...
impl NewsFetcher {
    ...
    pub async fn fetch_headlines(&self, symbol: &str) -> Result<Vec<NewsItem>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    loop {
        match reader.read_event() {
            ...
            Ok(Event::Text(ref e)) => {
                ...
                if in_pubdate { current_date.push_str(&text); }
...
fn guess_source_from_title(title: &str) -> String {
    let known_sources = [
        ...
        "Bloomberg Quint", "Economic Times", "Moneycontrol", "Business Standard",
        "Mint", "Livemint", "Yahoo Finance", "Investopedia", "CoinDesk", "CoinTelegraph",
        "BBC", "CNN", "The Hindu", "Times of India", "NDTV Profit",
    ];
...
<pubDate>Fri, 06 Jun 2025 20:00:00 GMT</pubDate>
    ...
    fn test_news_context_formatting() {
        let ctx = NewsContext {
            ...
            summary: "Positive sentiment overall. Institutional adoption driving prices.".to_string(),
            fetched_at: Utc::now(),
        };
    ...
    fn test_guess_source() {
        ...
        assert_eq!(guess_source_from_title("Oil prices drop - Reuters"), "Reuters");
        assert_eq!(guess_source_from_title("Unknown publication article"), "News");
    }
}
