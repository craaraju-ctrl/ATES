    ...
    pub fn new(path: impl AsRef<Path>) -> Result<Self, redb::Error> {
        ...
        for attempt in 0..6 {
            ...
            match Database::create(path) {
                Ok(db) => {
                    if attempt > 0 {
                        eprintln!("[MemoryStore] ✅ Recovered after {} attempts: {}", attempt + 1, path.display());
                    }
                ...
                Err(e) => {
                    ...
                    let is_lock = s.contains("AlreadyOpen") || s.to_lowercase().contains("already open") || s.contains("lock");
                    ...
                    if attempt < 5 && is_lock {
                        ...
                        std::thread::sleep(std::time::Duration::from_millis(350 + (attempt as u64) * 180));
                        continue;
                    }
                    if attempt < 3 {
        ...
        Err(last_err.unwrap_or_else(|| redb::Error::DatabaseAlreadyOpen))
    }
