        ...
        match self.state {
            ...
            CircuitState::HalfOpen => {
                ...
                if self.success_count >= self.success_threshold {
                    ...
                    self.failure_count = 0;
                    eprintln!("[CircuitBreaker] {}: circuit CLOSED after recovery", self.name);
                }
            }
            CircuitState::Open => {
        ...
        match self.state {
            CircuitState::Closed => {
                if self.failure_count >= self.failure_threshold {
                    ...
                    eprintln!("[CircuitBreaker] {}: circuit OPENED after {} failures", 
            ...
            CircuitState::HalfOpen => {
                ...
                self.state = CircuitState::Open;
                eprintln!("[CircuitBreaker] {}: circuit OPENED after half-open failure", self.name);
            }
            CircuitState::Open => {
...
impl CircuitBreakerManager {
    pub fn new() -> Self {
        ...
        managers.breakers.insert("ollama".to_string(), CircuitBreaker::new("ollama"));
        managers.breakers.insert("binance".to_string(), CircuitBreaker::new("binance"));
        managers.breakers.insert("coingecko".to_string(), CircuitBreaker::new("coingecko"));
        managers.breakers.insert("yahoo".to_string(), CircuitBreaker::new("yahoo"));
    ...
    {
        let breaker = self.breakers.get_mut(service)
            .expect("Circuit breaker not found for service");

        if !breaker.can_execute() {
            let msg = format!("[CircuitBreaker] {}: call rejected - circuit is OPEN", service);
            eprintln!("{}", msg);
            return Err(msg);
        }
