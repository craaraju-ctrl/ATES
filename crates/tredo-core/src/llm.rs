...
impl Default for LlmExecutor {
    fn default() -> Self {
        let model = std::env::var("OLLAMA_MODEL").unwrap_or_else(|_| "nemotron-3-nano:4b".to_string());
...
impl LlmExecutor {
    ...
    pub fn set_model(&mut self, model: String) {
        println!("[LlmExecutor] 🔄 Switching model from {} to {}", self.model, model);
        self.model = model;
    }
    
    /// Get all available models from Ollama
    pub async fn fetch_available_models(&mut self) -> Result<Vec<OllamaModel>, Box<dyn std::error::Error + Send + Sync>> {
        let base_url = self.endpoint.replace("/api/generate", "").replace("/api/chat", "");
        
        let res = self.client.get(&format!("{}/api/tags", base_url))
        ...
        self.available_models = tags_res.models.into_iter().map(|m| {
    ...
    pub async fn is_ollama_running(&self) -> bool {
        let base_url = self.endpoint.replace("/api/generate", "").replace("/api/chat", "");
        match self.client.get(&format!("{}/api/tags", base_url))
...
{{
  ...
  "recommendation": "summary recommendation"
    ...
    pub async fn summarize_news(&self, headlines: &[NewsItem], symbol: &str) -> String {
        ...
        if self.model.to_lowercase().contains("nano") || self.model.contains("3b") || self.model.contains("4b") {
...
SUMMARY:"#,
            ...
            .send().await
        {
            ...
            Err(e) => {
                ...
                return format!("Fetched {} headlines, summarization unavailable (timeout on local LLM).", headlines.len());
            }
        };
