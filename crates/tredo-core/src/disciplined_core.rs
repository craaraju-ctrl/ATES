...
pub fn check_risk_limits(context: &MarketContext, rules: &DisciplineRules) -> DisciplineCheck {
    ...
    let mut passed = true;
    let drawdown_pct = if context.equity > 0.0 { context.daily_pnl / context.equity } else { 0.0 };
    if drawdown_pct <= -rules.max_daily_drawdown {
        reasons.push(format!("Daily drawdown limit reached: {:.2}% (P&L: ₹{:.2} / Equity: ₹{:.2})",
            drawdown_pct.abs() * 100.0, context.daily_pnl, context.equity));
        passed = false;
    }
...
mod qa_tests {
    ...
    fn test_drawdown_triggers_at_real_percentage() {
        ...
        assert!(
            !result.passed,
            "4% drawdown should trigger 3% limit"
        );
    ...
    fn test_fibonacci_pivots_differ_from_classic() {
        ...
        assert!((fib.r1 - 97.64).abs() < 0.01, "Fibonacci R1 should be 97.64, got {}", fib.r1);
        assert!((fib.r2 - 102.36).abs() < 0.01, "Fibonacci R2 should be 102.36, got {}", fib.r2);
        assert!((fib.r3 - 110.0).abs() < 0.01, "Fibonacci R3 should be 110.0, got {}", fib.r3);
        assert!((fib.s1 - 82.36).abs() < 0.01, "Fibonacci S1 should be 82.36, got {}", fib.s1);
        assert!((fib.s2 - 77.64).abs() < 0.01, "Fibonacci S2 should be 77.64, got {}", fib.s2);
        assert!((fib.s3 - 70.0).abs() < 0.01, "Fibonacci S3 should be 70.0, got {}", fib.s3);
    }
}