...
impl AgentMemoryClient {
    pub fn new() -> Self {
        let base_url = env::var("AGENTMEMORY_URL").unwrap_or_else(|_| "http://localhost:3111".to_string());
        Self {
            client: Client::new(),
            base_url,
        }
    ...
    pub async fn recall(&self, query: &str) -> Result<Vec<String>, String> {
        ...
        Ok(data.into_iter().filter_map(|v| v.get("content").and_then(|c| c.as_str()).map(|s| s.to_string())).collect())
    }
}
