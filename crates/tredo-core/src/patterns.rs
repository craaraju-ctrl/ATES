...
pub fn format_patterns(patterns: &[CandlestickPattern]) -> String {
    ...
    let top: Vec<String> = patterns.iter().take(4).map(|p| {
        let arrow = match p.direction.as_str() {
            "bullish" => "🟢",
            "bearish" => "🔴",
            _ => "⚪",
        };
        format!("{} {} (strength: {:.0}%)", arrow, p.name, p.strength * 100.0)
    }).collect();
...
pub fn detect_patterns_multi_tf(tf_data: &[(&str, &[OhlcvBar])]) -> MultiTfPatternConfirmation {
    ...
    let bullish_count = patterns_by_tf.iter()
        .filter(|(_, pats)| pats.iter().any(|p| p.direction == "bullish"))
        .count();
    let bearish_count = patterns_by_tf.iter()
    ...
    let (preferred_direction, confidence_boost) = match (&bullish_conf, &bearish_conf) {
        ...
        (ConfirmationLevel::Moderate, ConfirmationLevel::Weak) => (Some("bullish".to_string()), 0.15),
        (ConfirmationLevel::Weak, ConfirmationLevel::Moderate) => (Some("bearish".to_string()), 0.15),
        ...
        (ConfirmationLevel::Moderate, ConfirmationLevel::None) => (Some("bullish".to_string()), 0.15),
        (ConfirmationLevel::None, ConfirmationLevel::Moderate) => (Some("bearish".to_string()), 0.15),
        _ => (None, 0.0),
    };
...
pub fn format_mtf_confirmation(mtf: &MultiTfPatternConfirmation) -> String {
    ...
    if !bullish_str.is_empty() { signals.push(bullish_str); }
    if !bearish_str.is_empty() { signals.push(bearish_str); }
    if signals.is_empty() {
    ...
    lines.push(format!("Timeframes: {}", mtf.timeframes_with_patterns.join(", ")));

    // Per-timeframe pattern list
    for (tf, pats) in &mtf.patterns_by_tf {
        if pats.is_empty() { continue; }
        let top = pats.iter().take(2).map(|p| {
            format!("{} ({:.0}%)", p.name, p.strength * 100.0)
        }).collect::<Vec<_>>().join(", ");
...
fn detect_doji(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    ...
    if is_doji_core(last) {
        let strength = 0.3;
        let direction = if last.close > bars.get(bars.len() - 2)?.close { "bullish" } else { "bearish" };
        Some(CandlestickPattern { name: "Doji".to_string(), direction: direction.to_string(), strength, bar_index: bars.len() - 1 })
    } else {
        None
    }
}
...
fn detect_hammer(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    ...
    if is_hammer {
        let direction = if is_bullish(last) { "bullish" } else { "bearish" };
        Some(CandlestickPattern { name: "Hammer".to_string(), direction: direction.to_string(), strength: 0.65, bar_index: bars.len() - 1 })
    } else {
        None
    }
}
...
fn detect_shooting_star(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    ...
    if is_shooting_star {
        Some(CandlestickPattern { name: "Shooting Star".to_string(), direction: "bearish".to_string(), strength: 0.65, bar_index: bars.len() - 1 })
    } else {
        None
    }
}
...
fn detect_marubozu(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    ...
    if body_pct >= 0.85 && uw_pct <= 0.05 && lw_pct <= 0.05 {
        let direction = if is_bullish(last) { "bullish" } else { "bearish" };
        Some(CandlestickPattern { name: if is_bullish(last) { "Bullish Marubozu" } else { "Bearish Marubozu" }.to_string(), direction: direction.to_string(), strength: 0.7, bar_index: bars.len() - 1 })
    } else {
        None
    }
}
...
fn detect_spinning_top(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    ...
    let body_pct = body / r;
    let is_spinning = body_pct >= 0.2 && body_pct <= 0.5
        && uw > body * 0.5 && lw > body * 0.5
        && (uw - lw).abs() <= body * 0.5;

    if is_spinning {
        Some(CandlestickPattern { name: "Spinning Top".to_string(), direction: "neutral".to_string(), strength: 0.25, bar_index: bars.len() - 1 })
    } else {
        None
    }
}
...
fn detect_bullish_engulfing(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    ...
    {
        Some(CandlestickPattern { name: "Bullish Engulfing".to_string(), direction: "bullish".to_string(), strength: 0.75, bar_index: bars.len() - 1 })
    } else {
        None
    }
}
...
fn detect_bearish_engulfing(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    ...
    if is_bullish(prev) && is_bearish(last)
        ...
        && body_size(last) > body_size(prev)
    {
        Some(CandlestickPattern { name: "Bearish Engulfing".to_string(), direction: "bearish".to_string(), strength: 0.75, bar_index: bars.len() - 1 })
    } else {
        None
    }
}
...
fn detect_bullish_harami(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    ...
    if is_bearish(prev) && is_bullish(last)
        ...
        && last.close < prev.open && last.open > prev.close
    {
        Some(CandlestickPattern { name: "Bullish Harami".to_string(), direction: "bullish".to_string(), strength: 0.55, bar_index: bars.len() - 1 })
    } else {
        None
    }
}
...
fn detect_bearish_harami(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    ...
    if is_bullish(prev) && is_bearish(last)
        ...
        && last.open < prev.close && last.close > prev.open
    {
        Some(CandlestickPattern { name: "Bearish Harami".to_string(), direction: "bearish".to_string(), strength: 0.55, bar_index: bars.len() - 1 })
    } else {
        None
    }
}
...
fn detect_piercing_line(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    ...
    {
        Some(CandlestickPattern { name: "Piercing Line".to_string(), direction: "bullish".to_string(), strength: 0.7, bar_index: bars.len() - 1 })
    } else {
        None
    }
}
...
fn detect_dark_cloud_cover(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    ...
    if is_bullish(prev) && is_bearish(last)
        ...
        && last.close > prev.open
    {
        Some(CandlestickPattern { name: "Dark Cloud Cover".to_string(), direction: "bearish".to_string(), strength: 0.7, bar_index: bars.len() - 1 })
    } else {
        None
    }
}
...
fn detect_morning_star(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    ...
    if is_bearish(b1)
        ...
        && b3.close > b2.close
    {
        Some(CandlestickPattern { name: "Morning Star".to_string(), direction: "bullish".to_string(), strength: 0.85, bar_index: bars.len() - 1 })
    } else {
        None
    }
}
...
fn detect_evening_star(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    ...
    if is_bullish(b1)
        ...
        && b3.close < b2.close
    {
        Some(CandlestickPattern { name: "Evening Star".to_string(), direction: "bearish".to_string(), strength: 0.85, bar_index: bars.len() - 1 })
    } else {
        None
    }
}
...
fn detect_three_white_soldiers(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    ...
    if is_bullish(b1) && is_bullish(b2) && is_bullish(b3)
        && b1.close > b1.open && b2.close > b2.open && b3.close > b3.open
        && b2.close > b1.close && b3.close > b2.close
        && body_size(b1) > 0.0 && body_size(b2) > 0.0 && body_size(b3) > 0.0
    {
        Some(CandlestickPattern { name: "Three White Soldiers".to_string(), direction: "bullish".to_string(), strength: 0.8, bar_index: bars.len() - 1 })
    } else {
        None
    }
}
...
fn detect_three_black_crows(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    ...
    if is_bearish(b1) && is_bearish(b2) && is_bearish(b3)
        && b1.close < b1.open && b2.close < b2.open && b3.close < b3.open
        && b2.close < b1.close && b3.close < b2.close
        && body_size(b1) > 0.0 && body_size(b2) > 0.0 && body_size(b3) > 0.0
    {
        Some(CandlestickPattern { name: "Three Black Crows".to_string(), direction: "bearish".to_string(), strength: 0.8, bar_index: bars.len() - 1 })
    } else {
        None
    }
}
...
mod tests {
    ...
    fn bar(open: f64, high: f64, low: f64, close: f64) -> OhlcvBar {
        OhlcvBar { timestamp: String::new(), open, high, low, close, volume: 1000.0 }
    }
    ...
    fn test_doji_detection() {
        let bars = vec![bar(100.0, 110.0, 90.0, 105.0), bar(100.0, 101.0, 99.0, 100.1)];
    ...
    fn test_hammer_detection() {
        ...
        let bars = vec![bar(100.0, 105.0, 95.0, 102.0), bar(102.0, 102.3, 95.0, 102.5)];
        let patterns = detect_patterns(&bars);
    ...
    fn test_shooting_star_detection() {
        ...
        let bars = vec![bar(100.0, 105.0, 95.0, 102.0), bar(101.0, 110.0, 100.7, 100.5)];
        let patterns = detect_patterns(&bars);
    ...
    fn test_marubozu_detection() {
        ...
        let bars = vec![bar(100.0, 102.0, 98.0, 101.0), bar(100.0, 105.05, 100.0, 105.0)];
        let patterns = detect_patterns(&bars);
    ...
    fn test_bullish_engulfing() {
        ...
        let bars = vec![bar(105.0, 106.0, 99.0, 100.0), bar(99.0, 108.0, 98.0, 107.0)];
        let patterns = detect_patterns(&bars);
    ...
    fn test_bearish_engulfing() {
        ...
        let bars = vec![bar(100.0, 101.0, 99.0, 100.5), bar(101.0, 102.0, 95.0, 96.0)];
        let patterns = detect_patterns(&bars);
    ...
    fn test_format_patterns_with_data() {
        let patterns = vec![
            CandlestickPattern { name: "Bullish Engulfing".to_string(), direction: "bullish".to_string(), strength: 0.75, bar_index: 0 },
        ];
    ...
    fn test_not_detecting_false_patterns() {
        ...
        let bars: Vec<OhlcvBar> = (0..10).map(|i| {
            bar(100.0 + i as f64, 102.0 + i as f64, 98.0 + i as f64, 101.0 + i as f64)
        }).collect();
        let patterns = detect_patterns(&bars);
