...
// within the ates-core crate. Paper/live share the exact same direction type.
...
impl std::fmt::Display for OrderStatus {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            OrderStatus::PartiallyFilled { filled_qty } => write!(f, "PARTIALLY_FILLED (qty={})", filled_qty),
...
impl PaperEngine {
    ...
    pub async fn place_order(
        ...
        // Generate order ID
        let order_id = format!("ORD-{:06}", self.order_counter.fetch_add(1, Ordering::SeqCst));
        let now = Utc::now();
        ...
        let fill_price = match request.order_type {
            ...
            OrderType::Limit => {
                request.price.unwrap_or(market_price)
            }
        ...
        let position = Position {
            id: format!("POS-{:06}", self.trade_counter.fetch_add(1, Ordering::SeqCst)),
    ...
    pub async fn update_price(
        ...
        while i < portfolio.positions.len() {
            ...
            if pos_clone.stop_loss > 0.0 {
                ...
                if stop_hit {
                    let closed_trade = self.close_position_internal(
                        &mut portfolio, i, CloseReason::StopLoss, pos_clone.stop_loss,
                    );
            ...
            if pos_clone.take_profit > 0.0 {
                ...
                if tp_hit {
                    let closed_trade = self.close_position_internal(
                        &mut portfolio, i, CloseReason::TakeProfit, pos_clone.take_profit,
                    );
                    closed.push(closed_trade);
                    continue;
                }
    ...
    pub async fn close_position(&self, position_id: &str, exit_price: f64) -> Result<ClosedTrade, String> {
        let mut portfolio = self.portfolio.write().await;
        let idx = portfolio.positions.iter().position(|p| p.id == position_id)
    ...
    fn close_position_internal(
        ...
        let (realized_pnl, realized_pnl_pct) = Self::calculate_pnl(
            pos.direction,
            pos.entry_price,
            exit_price,
            pos.qty,
        ...
        if portfolio.equity < portfolio.max_drawdown {
            ...
            let dd = (portfolio.max_drawdown - portfolio.initial_balance) / portfolio.initial_balance * 100.0;
    ...
    pub async fn check_risk(&self, symbol: &str, estimated_cost: f64) -> RiskCheckResult {
        ...
        if portfolio.daily_pnl < -daily_loss_limit {
            ...
            result.warnings.push(format!(
                ...
                portfolio.daily_pnl.abs(), daily_loss_limit, self.config.max_daily_loss_pct
            ));
        }
        ...
        let existing_exposure: f64 = portfolio.positions
            .iter()
        ...
        // Portfolio heat
        let total_risk: f64 = portfolio.positions
            .iter()
            .map(|p| {
    ...
    pub async fn get_summary(&self) -> PortfolioSummary {
        ...
        let margin_used: f64 = p.positions
            .iter()
...
pub trait BrokerAdapter: Send + Sync {
    ...
    async fn close_position(&self, position_id: &str, exit_price: f64) -> Result<ClosedTrade, String>;
...
impl BrokerAdapter for PaperBroker {
    ...
    async fn place_order(&self, request: OrderRequest, market_price: f64) -> Result<String, String> {
        ...
        let risk = self.engine.check_risk(&request.symbol, estimated_cost).await;
        if !risk.passed {
...
impl BrokerAdapter for ZerodhaKiteBroker {
    async fn connect(&self) -> Result<(), String> {
        // TODO: Implement Kite Connect API authentication
        // POST /session/token with api_key, api_secret, request_token
        Err("Zerodha Kite live broker not yet implemented. Use PaperBroker for testing.".to_string())
    }
    ...
    async fn place_order(&self, _request: OrderRequest, _market_price: f64) -> Result<String, String> {
    ...
    pub async fn set_mode(&self, mode: TradingMode) -> Result<(), String> {
        match mode {
            ...
            TradingMode::Live => {
                ...
                if brokers.is_empty() {
                    return Err("No live broker registered. Configure API credentials first.".to_string());
                }
