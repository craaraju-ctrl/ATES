...
/// Brute-force search is O(n·d) which is fine for <10,000 entries.
...
impl VectorMemory {
    ...
    /// Generate an embedding for the given text via Ollama and store a new entry.
    pub async fn store(
        &mut self,
        episode_id: &str,
        symbol: &str,
    ...
    /// Find the top-k most similar entries to a given text (by embedding the query first).
    pub async fn search(
        &self,
        query_text: &str,
        top_k: usize,
        ...
        let mut scored: Vec<(&VectorEntry, f64)> = self.entries
            .values()
            .map(|entry| {
    ...
    /// Find the top-k most similar entries to a given embedding vector directly.
    /// Used when the query embedding is already computed.
    ...
    /// Save all entries to a JSON file on disk.
    ...
    fn load_from_disk(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        if std::path::Path::new(&self.db_path).exists() {
            ...
            println!("[VectorMemory] ✅ Loaded {} entries from disk", self.entries.len());
        }
        Ok(())
    }
}
...
mod tests {
    ...
    fn test_search_by_vector_with_data() {
        ...
        mem.entries.insert("ep1".to_string(), VectorEntry {
        ...
        mem.entries.insert("ep2".to_string(), VectorEntry {
