...
use chrono::Utc;
use ates_autonomous::state::SharedState;
use ates_autonomous::AutonomousOrchestrator;
use ates_core::{TradeDirection, OhlcvBar, PivotMethod, calculate_pivot_points, calculate_confluence_score, MarketContext, Agent};
use ates_core::episode::{TradingEpisode, MarketStateSnapshot, ReasoningStep};
        ...
        tokio::select! {
            ...
            _ = sleep(Duration::from_secs(5)) => {
                ...
                for symbol in &assets {
                    let is_crypto = matches!(symbol.as_str(), "BTC" | "ETH" | "SOL");
...
// ── Medium Loop (every 5m): full pipeline, market scan ─────────────────────
            ...
            _ = sleep(Duration::from_secs(300)) => {
                ...
                for symbol in &assets {
                    ...
                    match orchestrator.run_full_pipeline(symbol, direction, price, stop, target).await {
                        Ok(summary) => {
                            if summary.executed {
                                println!("[MediumLoop] ✅ Trade EXECUTED | {}", summary.reason);
                                // Capture episode on execution
                                capture_trade_episode(&orchestrator, &summary).await;
                            } else {
                                println!("[MediumLoop] ⏸ No trade | {}", summary.reason);
                            }
                        }
                        Err(e) => eprintln!("[MediumLoop] ❌ Pipeline error for {}: {}", symbol, e),
                    }
                }
                ...
                for symbol in &assets {
                    ...
                    tokio::spawn(async move {
                        let fetcher = ates_core::NewsFetcher::new(c);
    ...
    loop {
        tokio::select! {
            ...
            _ = sleep(Duration::from_secs(86400)) => {
                    ...
                    if let Ok(mut episode) = serde_json::from_str::<TradingEpisode>(&json) {
                        ...
                        if episode.outcome.is_some() && episode.reflection.is_none() {
                            let reflection = orchestrator.reflector
                                ...
                                .await
                                .unwrap_or_else(|e| ates_core::PostTradeReflection {
                                    timestamp: Utc::now(),
                ...
                let meta = ates_autonomous::meta_control::MetaControlAgent::new(state.clone());
...
async fn capture_trade_episode(
    orchestrator: &AutonomousOrchestrator,
    summary: &ates_autonomous::types::PipelineSummary,
) {
    if let Some(ref signal) = summary.final_signal {
        ...
        let regime_str = match *regime {
            Some(ates_autonomous::types::MarketRegime::TrendingBull) => "TrendingBull",
            Some(ates_autonomous::types::MarketRegime::TrendingBear) => "TrendingBear",
            Some(ates_autonomous::types::MarketRegime::Ranging) => "Ranging",
            Some(ates_autonomous::types::MarketRegime::Volatile) => "Volatile",
            Some(ates_autonomous::types::MarketRegime::LowLiquidity) => "LowLiquidity",
            None => "Unknown",
        };
        drop(regime);
        ...
        let episode = TradingEpisode {
            ...
            market_state: MarketStateSnapshot {
                ...
                pivot: 0.0, r1: 0.0, s1: 0.0, // filled from signal context
                ...
                portfolio_heat: portfolio.open_positions.iter().map(|p| p.risk_amount).sum::<f64>() / portfolio.total_equity.max(1.0),
                consecutive_losses: portfolio.consecutive_losses,
                daily_pnl_pct: portfolio.daily_pnl_pct,
            },
            action: if signal.direction == ates_core::TradeDirection::Long { "BUY".to_string() } else { "SELL".to_string() },
            ...
            reasoning_trace: vec![ReasoningStep {
                ...
                input_summary: format!("Market analysis for {} @ {:.2}", signal.symbol, signal.entry_price),
        ...
        if let Ok(json) = serde_json::to_string(&episode) {
            ...
            orchestrator.state.latest_episode.write().await.insert(signal.symbol.clone(), ep_id.clone());
            ...
            if let Err(e) = vm.store(&ep_id, &signal.symbol, &store_text, None, &orchestrator.state.llm).await {
                eprintln!("[VectorMemory] ⚠ Failed to embed episode {}: {}", ep_id, e);
            } else {
                println!("[VectorMemory] 🧠 Embedded episode {} ({} dims)", ep_id, vm.len());
            }
        }
    }
}
...
use ates_autonomous::types::PortfolioState;
use ates_autonomous::state::TimeframeData;

async fn fetch_price(client: &reqwest::Client, symbol: &str, is_crypto: bool) -> Result<f64, Box<dyn std::error::Error + Send + Sync>> {
    if is_crypto {
...
pub async fn fetch_binance_klines(
    ...
    let resp: Vec<Vec<serde_json::Value>> = client.get(&url)
        .header("User-Agent", "Mozilla/5.0")
        .timeout(Duration::from_secs(5))
        .send().await?
        .json().await?;
    ...
    for kline in resp {
        ...
        let open  = kline[1].as_str().and_then(|s| s.parse::<f64>().ok()).unwrap_or(0.0);
        let high  = kline[2].as_str().and_then(|s| s.parse::<f64>().ok()).unwrap_or(0.0);
        let low   = kline[3].as_str().and_then(|s| s.parse::<f64>().ok()).unwrap_or(0.0);
        let close = kline[4].as_str().and_then(|s| s.parse::<f64>().ok()).unwrap_or(0.0);
        let volume = kline[5].as_str().and_then(|s| s.parse::<f64>().ok()).unwrap_or(0.0);
        let dt = chrono::DateTime::from_timestamp_millis(open_time).unwrap_or_else(chrono::Utc::now);
        bars.push(OhlcvBar { timestamp: dt.to_rfc3339(), open, high, low, close, volume });
    }
    Ok(bars)
}

pub async fn fetch_yahoo_ohlcv(client: &reqwest::Client, symbol: &str) -> Result<Vec<OhlcvBar>, Box<dyn std::error::Error + Send + Sync>> {
    let yahoo_symbol = match symbol {
        "NIFTY"    => "^NSEI",
        "RELIANCE" => "RELIANCE.NS",
        other      => other,
    ...
    let timestamps = result["timestamp"].as_array()
        .map(|a| a.iter().filter_map(|v| v.as_i64()).collect::<Vec<_>>()).unwrap_or_default();
    let quote = &result["indicators"]["quote"][0];
    let opens: Vec<f64> = quote["open"].as_array().map(|a| a.iter().filter_map(|v| v.as_f64()).collect()).unwrap_or_default();
    let highs: Vec<f64> = quote["high"].as_array().map(|a| a.iter().filter_map(|v| v.as_f64()).collect()).unwrap_or_default();
    let lows: Vec<f64> = quote["low"].as_array().map(|a| a.iter().filter_map(|v| v.as_f64()).collect()).unwrap_or_default();
    let closes: Vec<f64> = quote["close"].as_array().map(|a| a.iter().filter_map(|v| v.as_f64()).collect()).unwrap_or_default();
    let volumes: Vec<f64> = quote["volume"].as_array().map(|a| a.iter().filter_map(|v| v.as_f64()).collect()).unwrap_or_default();

    let n = timestamps.len().min(opens.len()).min(highs.len()).min(lows.len()).min(closes.len()).min(volumes.len());
    let mut bars = Vec::with_capacity(n);
    for i in 0..n {
        let dt = chrono::DateTime::from_timestamp(timestamps[i], 0).unwrap_or_else(chrono::Utc::now);
        bars.push(OhlcvBar { timestamp: dt.to_rfc3339(), open: opens[i], high: highs[i], low: lows[i], close: closes[i], volume: volumes[i] });
    }
    Ok(bars)
}

fn update_ohlcv_history(history: &mut Vec<OhlcvBar>, price: f64, now: &chrono::DateTime<chrono::Utc>) {
    if history.is_empty() {
        history.push(OhlcvBar { timestamp: now.to_rfc3339(), open: price, high: price, low: price, close: price, volume: 0.0 });
        return;
    }
    ...
    let last_time = chrono::DateTime::parse_from_rfc3339(&last_ts)
        .map(|dt| dt.with_timezone(&chrono::Utc)).unwrap_or(*now);

    if (*now - last_time).num_seconds() >= 60 {
        history.push(OhlcvBar { timestamp: now.to_rfc3339(), open: last_close, high: price, low: price, close: price, volume: 0.0 });
        while history.len() > 200 { history.remove(0); }
    } else {
...
async fn fetch_multi_tf_binance(
    client: &reqwest::Client, symbol: &str, state: &SharedState
    ...
    for (interval, limit) in &intervals {
        match fetch_binance_klines(client, symbol, interval, *limit).await {
            Ok(bars) if !bars.is_empty() => {
                ...
                let pivots = calculate_pivot_points(close_price * 1.01, close_price * 0.99, close_price * 0.998, PivotMethod::Classic);
                let context = MarketContext {
                    symbol: symbol.to_string(), current_price: close_price,
                    high: close_price * 1.01, low: close_price * 0.99,
                    previous_close: close_price * 0.998, timestamp: Utc::now(),
                    daily_pnl: 0.0, equity, consecutive_losses: 0, is_red_folder_day: false, trend_direction: None,
                };
                let confluence = calculate_confluence_score(&context, &pivots);
                println!("[MTF] {} {}: {} bars | Pivot={:.2} | Conf={:.1}%", symbol, interval, bars.len(), pivots.pivot, confluence * 100.0);
                results.push(TimeframeData {
                    timeframe: interval.to_string(), ohlcv: bars,
                    pivots: Some(pivots), confluence, last_updated: Utc::now(),
                });
            }
    ...
    match fetch_yahoo_ohlcv(client, symbol).await {
        Ok(bars) if !bars.is_empty() => {
            ...
            Ok(vec![TimeframeData {
                timeframe: "1m".to_string(), ohlcv: bars,
                pivots: Some(pivots), confluence, last_updated: Utc::now(),
            }])
        }
        _ => Ok(vec![]),
    ...
    for symbol in assets {
        ...
        tokio::spawn(async move {
            let tf_data = if is_crypto {
                fetch_multi_tf_binance(&tf_client, &tf_symbol, &state_clone).await.unwrap_or_default()
            } else {
                fetch_multi_tf_yahoo(&tf_client, &tf_symbol, &state_clone).await.unwrap_or_default()
            };
            if !tf_data.is_empty() {
...
pub async fn update_multi_tf_data(
    ...
    let tf_data = if is_crypto {
        fetch_multi_tf_binance(client, symbol, &orchestrator.state).await.unwrap_or_default()
    } else {
        fetch_multi_tf_yahoo(client, symbol, &orchestrator.state).await.unwrap_or_default()
    };
    if !tf_data.is_empty() {
...
async fn execute_due_tasks(orchestrator: &AutonomousOrchestrator, now: &chrono::DateTime<chrono::Utc>) {
    let tasks = orchestrator.state.agent_tasks.read().await;
    let due_tasks: Vec<(usize, String)> = tasks.iter().enumerate()
    ...
    for (idx, name) in due_tasks {
        ...
        match name.as_str() {
            "market_scan" => { let _ = orchestrator.scanner.scan_watchlist().await; }
            "position_monitor" => { let _ = orchestrator.execution.run(None).await; }
            "portfolio_review" => {
                let p = orchestrator.state.portfolio.read().await;
                println!("[Review] Daily P&L: {:+.2} | Trades: {}", p.daily_pnl, p.total_trades_today);
                drop(p);
            }
            "goal_review" => {
                ...
                println!("[Goals] 📊 Mode: {:?} | Current: {:+.2}%", goals.mode, p.daily_pnl_pct * 100.0);
                drop(p);
                drop(goals);
            }
            _ => {}
...
fn log_portfolio_snapshot(portfolio: &PortfolioState, state: &SharedState) {
    ...
    println!("   [Mode] {:?} | Target: {:+.2}% | Current: {:+.2}% | Trades: {}/{}",
        goals.mode, goals.daily_target_pnl_pct * 100.0, portfolio.daily_pnl_pct * 100.0,
        portfolio.total_trades_today, goals.max_daily_trades);
}
...
async fn log_portfolio_snapshot_full(orchestrator: &AutonomousOrchestrator) {
    ...
    let goals = orchestrator.state.trading_goals.read().await;
    println!("\n📊 [Portfolio] Equity: ₹{:.2} | Cash: ₹{:.2} | Positions: {} | P&L: ₹{:.2} | DD: {:.2}%",
        p.total_equity, p.cash_balance, p.open_positions.len(), p.daily_pnl, p.max_drawdown_today * 100.0);
    println!("   [Mode] {:?} | Target: {:+.2}% | Current: {:+.2}% | Trades: {}/{}",
        goals.mode, goals.daily_target_pnl_pct * 100.0, p.daily_pnl_pct * 100.0,
        p.total_trades_today, goals.max_daily_trades);
    if !p.open_positions.is_empty() {
        for pos in &p.open_positions {
            println!("   • {} {} {} qty={:.0} entry={:.2} cur={:.2} P&L=₹{:.2}",
                pos.symbol, if pos.direction == ates_core::TradeDirection::Long { "LONG" } else { "SHORT" },
