...
> **Lightweight FastAPI microservice** that exposes the Kronos foundation model as a REST API for the ATES autonomous trading system.
...
```mermaid
graph TB
    subgraph "ATES Rust Backend"
        ORCH[ATES Orchestrator]
...
The Rust side (`ates-core/src/kronos_client.rs`) calls this service over HTTP.
...
| **Timeout** | ATES client has a 5-second timeout; service typically responds in 100-500ms |
| **Graceful Degradation** | If Kronos is unavailable, ATES falls back to Neutral trend direction |
